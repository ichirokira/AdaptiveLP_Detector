...
CLASSES = {'Car' : 0, 'Van' : 1, 'Truck' : 2,
                     ...
                     'Misc':7, 'DontCare':8}
def get_kitti_dicts(img_dir, label_dir, mode="train",disp_dir=None, using_mask=False):

    labels_list = list(glob(label_dir + "/*.txt"))
    length = int(len(labels_list) * 0.7)
    if mode =="train":
    ...
    for i, label_file in enumerate(labels_list):
        ...
        if disp_dir is not None:
            disp_dir = disp_dir + "/"+file_name+".png"
        img = cv2.imread(file_dir)
        disp = cv2.imread(disp_dir)
        if disp is None:
            continue
        if img is None:
            continue
